...
```
此命令启动接收方，指定自身端口为 `56007`，接收 `59606` 端口的接收方发过来的数据，输出到 `output` 文件中，滑动窗口大小为 `1000` 字节。
...
```
此命令启动发送方，指定自身端口为 `59606`，向 `56007` 端口的接收方传输随机数字，滑动窗口大小为 `1000` 字节，超时重传时间间隔为 `500ms`，正向丢包率 `0.1`，反向丢包率 `0.1`。
